...
class Backbone(torch.nn.Module):
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        for block in self._config.blocks:
            self.build_block_output(block, feature_group_values)
        backbone_output = torch.cat(list(self._block_outputs), dim=1)
    ...
    def build_block_output(self, block_config: BlockConfig, feature_group_values: Dict[str, Any]) -> torch.Tensor:
        ...
        if module_type == "mlp":
            ...
            block_output = MLP(in_features=block_input.shape[1], **module_config)(block_input)
        elif module_type == "din":
            block_input = block_inputs[0]
            sequence_dim = list(block_input.values())[0].shape[2]
            block_output = DIN(sequence_dim=sequence_dim, query_dim=sequence_dim, **module_config)(block_input)
        else:
            raise ValueError(
